...
```bash
git clone https://github.com/narduk-enterprises/tideye.git my-project
cd my-project
pnpm install
...
| ---------------------- | ---------- | -------------- |
| `tideye` | `dev`      | Shared secrets |
| `tideye` | `dev_auth` | `example-auth` |

**First-time setup for example-auth:**

```bash
cd apps/example-auth
doppler setup --project tideye --config dev_auth --no-interactive
```
